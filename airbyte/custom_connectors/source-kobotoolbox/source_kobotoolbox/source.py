...
class KoboToolStream(HttpStream, IncrementalMixin):
    ...
    def __init__(self, config: Mapping[str, Any], form_id, response, schema, name, pagination_limit, auth_token, **kwargs):
        super().__init__()
    ...
    def get_json_schema(self):
        ...
        filtered_dict = {key: {"type": [value, "null"]}  for key, value in properties.items() if value is not None and key !="null" and key is not None}
        self.schema["properties"] = {**self.schema["properties"], **filtered_dict}
        return self.schema 
