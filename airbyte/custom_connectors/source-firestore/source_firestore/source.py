...
class Helpers(object):
    ...
    @staticmethod
    def get_documents(project_id: str, collection_name: str) -> str:
        return f"{Helpers.url_base}projects/{project_id}/databases/(default)/documents/{collection_name}"
...
class FirestoreStream(HttpStream, ABC):
    ...
    def __init__(self, authenticator: TokenAuthenticator, collection_name: str, response):
        super().__init__(
        ...
        for entry in data:
            if "document" in entry:
                ...
                if "fields" in document:
                    ...
                    result.update( self.parse_json_data(document["fields"]))
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        ...
        first_record= self.response[0]
        if first_record:
            if "fields" in first_record:
               for key, value in first_record["fields"].items():
                   result["properties"][key] = {"type": [list(value.keys())[0].rstrip("Value"), "string", "null"]} 
        if self.cursor_key:
...
class IncrementalFirestoreStream(FirestoreStream, IncrementalMixin):
    ...
    def __init__(self, authenticator: TokenAuthenticator, collection_name: str, response, cursor_field_possibilities: List[str] = []):
        super().__init__(authenticator=authenticator, collection_name=collection_name, response=response)
...
class Collection(IncrementalFirestoreStream):
    ...
    def __init__(self, authenticator: TokenAuthenticator, collection_name: str, config: Mapping[str, Any]):
        ...
        self.collection_name = collection_name
        res = requests.get(Helpers.get_documents(self.project_id, self.collection_name), headers=authenticator.get_auth_header())
        res.raise_for_status()
        json = res.json()
        response = json.get("documents", [])
        super().__init__(authenticator, collection_name=collection_name, cursor_field_possibilities=config["cursor_field_possibilities"], response =response)
